...
const size = 20;
const bwidth = 2;
var Grade = JSON.parse(localStorage.ChildDetails).Grade;
Grade = Grade > 0 && Grade <= 4 ? 1 : Grade > 4 && Grade <= 8 ? 2 : 3;
const mazeHeight = 200 * (Grade /2);
const mazeWidth = 300 * (Grade /3);
...
function updateEmo(lr) {
	...
	if (lr) {
		...
		if (thingie.offsetLeft > mazeWidth) {
			emo.innerHTML = "";
			home.innerHTML = "🥳";
			$('#maze').attr({ completed:true })
		}
		else
			$('#maze').attr({ completed:false })
